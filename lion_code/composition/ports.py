...
class MemoryTurnPort:
    """延迟绑定 SessionMemoryCoordinator 的窄 Memory structural port。"""

    def __init__(self) -> None:
        self._coordinator: Any | None = None
        self._semantic_extractor: Callable[[Any, str, str], Any] | None = None

    def bind(
        self,
        coordinator: Any,
        *,
        semantic_extractor: Callable[[Any, str, str], Any] | None = None,
    ) -> None:
        self._coordinator = coordinator
        self._semantic_extractor = semantic_extractor

    @property
    def _memory_coordinator(self) -> Any:
        return self._require_coordinator().memory_coordinator

    @_memory_coordinator.setter
    def _memory_coordinator(self, value: Any) -> None:
        self._require_coordinator().memory_coordinator = value

    def set_semantic_extractor(
        self,
        extractor: Callable[[Any, str, str], Any] | None,
    ) -> None:
        """更新测试 seam；默认仍使用 Coordinator 自己的提取实现。"""

        self._semantic_extractor = extractor

    @property
    def _memory_injector(self) -> Any:
        if self._coordinator is None:
            raise RuntimeError("Session Memory 尚未初始化")
        return self._coordinator.memory_injector

    @property
    def _turn_memory_overlays(self) -> tuple[Any, ...]:
        if self._coordinator is None:
            raise RuntimeError("Session Memory 尚未初始化")
        return self._coordinator.turn_memory_overlays

    @_turn_memory_overlays.setter
    def _turn_memory_overlays(self, value: tuple[Any, ...]) -> None:
        if self._coordinator is None:
            raise RuntimeError("Session Memory 尚未初始化")
        self._coordinator.turn_memory_overlays = value

    @property
    def _last_memory_injection(self) -> Any:
        if self._coordinator is None:
            raise RuntimeError("Session Memory 尚未初始化")
        return self._coordinator.last_memory_injection

    @_last_memory_injection.setter
    def _last_memory_injection(self, value: Any) -> None:
        if self._coordinator is None:
            raise RuntimeError("Session Memory 尚未初始化")
        self._coordinator.last_memory_injection = value

    def _prepare_turn_memory_snapshot(self, user_message: str) -> None:
        self._require_coordinator()._prepare_turn_memory_snapshot(user_message)

    def _build_turn_memory_overlays(self) -> tuple[Any, ...]:
        return self._require_coordinator()._build_turn_memory_overlays()
    ...
    async def _update_session_memory_after_turn(
        ...
        if self._semantic_extractor is None:
            await coordinator._update_session_memory_after_turn(
                user_message,
                turn_start_index,
            )
            return
        await coordinator._update_session_memory_after_turn(
            user_message,
            turn_start_index,
            semantic_extractor=self._semantic_extractor,
        )

    def _reload_project_memory(self) -> None:
        self._require_coordinator()._reload_project_memory()

    def _reload_session_memory(self) -> None:
        self._require_coordinator()._reload_session_memory()
