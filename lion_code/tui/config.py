...
# Lion 本地化:配置落 ~/.lion-code(替代上游 TauPaths 的 ~/.tau)。
_DEFAULT_HOME = Path.home() / ".lion-code"


def tui_settings_path(home: Path | None = None) -> Path:
    """Return the durable TUI settings path."""
    return (home or _DEFAULT_HOME) / "tui.json"


def load_tui_settings(home: Path | None = None) -> TuiSettings:
...
def save_tui_settings(settings: TuiSettings, home: Path | None = None) -> Path:
