...
| **持久化目标** | 长期自治目标、尝试次数、重试阶段与任务调度状态。 | 对话历史、消息树、模型/思考档位配置、压缩摘要。 |
...
### 2. Session 会话恢复 (`AgentRuntime.restore`)
```
1. 仓库加载 (SessionRepository.load(session_id))
   └─ 从 JSONL 读取全部 Entry，通过 SessionState.from_entries() 自底向上折叠：
      被 CompactionEntry 覆盖的旧消息替换为单个 Previous conversation summary 消息。
...
* **分布式/跨节点锁 (Distributed Lock)**：`JsonCheckpointStore` 依赖本地文件系统的原子重命名 (`os.replace`) 保证单机并发安全，未实现跨节点分布式协调。
