...
| 环境缺陷 | WSL 编辑写 LF / Windows checkout 写 CRLF 行尾污染 | `core.autocrlf=true`：WSL 工具改过的文件提交时整文件被当改动（数千行假 diff）。提交前用 Windows 侧 git 复核 `git diff --stat`，必要时 `git checkout HEAD -- <file>` 恢复后重改。属本机环境，不误记为产品路径。 |
