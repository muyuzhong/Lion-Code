...
## Planning and authorization contract

An explicit request to implement authorizes its stated scope. A request only to discuss, diagnose, review, or plan does not authorize implementation; task-creation consent alone does not either. Respect any explicit user requirement to approve the plan before implementation.

Do not require a fresh user response solely to complete planning. Once required artifacts are reviewed, proceed under existing implementation authorization unless a material decision remains, the scope changes, or the user requested an approval gate.

When an unresolved product, scope, UX, compatibility, risk, or acceptance decision materially changes the result, ask the highest-value question and hold dependent implementation. Continue independent authorized evidence work; do not treat minor implementation choices as mandatory approval gates.
...
2. Inspect available evidence before asking questions:
   ...
   - directly relevant task artifacts and research; do not read unrelated session history, caches, or credentials
3. Separate what you found into:
   - confirmed facts
   - product intent still needed from the user
   - scope or risk decisions still needed from the user
   - likely out-of-scope items
4. If a material user-owned decision remains, ask the highest-value question with a recommendation and trade-off. Hold dependent work while continuing independent authorized work.
...
8. Present the final planning summary. If implementation is already authorized, scope is unchanged, and no explicit user review gate remains, run `task.py start` and continue.
9. Otherwise obtain the missing implementation approval or material decision before starting. Revisit approval only when changes exceed existing authorization or alter a user-approved decision.
...
Recommendations do not resolve material user-owned decisions by themselves. For routine choices within clear authorization, use established conventions and state reasonable assumptions rather than manufacturing a product decision.

Do not manufacture clarification questions when the request and repository evidence already resolve every decision. In that case, present the final planning summary and proceed under existing implementation authorization.

Final artifact review is required for a formal task; a new user approval is required only when authorization is missing, a material decision changes, or the user explicitly requested that gate. Task-creation consent alone is insufficient.
...
Lightweight tasks may omit `design.md` and `implement.md`; they may not skip evidence inspection, requirement convergence, final artifact review, or implementation authorization. Existing explicit implementation authorization counts.
...
Before declaring planning ready or running `task.py start`, review `prd.md` against the final structure described in the artifact rules above. Edit only where needed to resolve duplication, missing decisions, or temporary sections; do not rewrite an already conforming artifact merely to satisfy the gate.
...
- Implementation authorization covers the final scope, and any explicit user approval gate has been satisfied.

Proceed only when both planning readiness and implementation authorization are satisfied; do not require repeated approval of unchanged, already authorized work.
