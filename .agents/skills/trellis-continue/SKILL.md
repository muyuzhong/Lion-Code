...
Use this skill for an explicit request to resume a Trellis task. Existing task state alone does not activate it. Resume at the appropriate step in `.trellis/workflow.md`, preserving existing authorization.
...
- `status=planning` + required artifacts complete + required jsonl curated or inline mode → **1.4** (review readiness; run `task.py start` under existing implementation authorization unless a material decision or explicit user approval gate remains)
- `status=in_progress` + implementation not started → **2.1**
- `status=in_progress` + implementation done, not yet checked → **2.2**
- `status=in_progress` + check passed → **3.3** (spec update) → **3.4** (check commit authorization; uncommitted delivery is valid)
