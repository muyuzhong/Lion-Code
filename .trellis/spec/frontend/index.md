...
桌面 Renderer（`desktop/src/renderer/`，React + TypeScript + electron-vite + vitest）编码规范入口。
...
- [ ] 协议层（`desktop/src/shared/chat.ts`）改动必须配套 `chatProtocol.test.ts` 契约级单测
...
- [ ] `cd desktop && npm test` 全过（含新增契约单测）
- [ ] `npm run build`（含 tsc -b 类型检查）通过
- [ ] `npm run typecheck` 与 Electron Playwright 对应 project 通过；发布改动额外运行 `npm run verify:delivery`
