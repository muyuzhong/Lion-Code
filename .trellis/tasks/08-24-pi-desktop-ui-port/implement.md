...
1. 增加 `lucide-react`，按上游源码建立 PI 深色 Token 与模块化样式入口。
...
6. 忠实复现右侧工作面板 Shell 与空资源态，不引入资源 Runtime。
7. 收敛设置、空状态、连接/错误状态和主题表现。
8. 增加组件契约测试、窗口尺寸断言与关键状态截图，并在 1636x1050 对照用户提供的 PI-Desktop 截图。
9. 补齐会话标题的 canonical `LabelEntry` 写入、REST rename 契约，以及 assistant-ui `/` Skill TriggerPopover；回归 Composer 焦点样式。
...
- 审查所有新增图标、样式和组件是否确实被使用；工作面板只保留截图要求的 Shell 和明确空状态。
