...
| 顺序 | 切片 | 状态 | 可验证结果 |
| --- | --- | --- | --- |
| S3 | `subagent_factory` | 当前子任务 | 子 Agent 与 Skill fork 的构造、工具筛选与懒导入边界独立 |
| S4 | `learning_runtime` | 待创建 | 已有特征测试覆盖的学习运行时职责独立 |
...
父任务只管理路线、子任务映射和最终集成验收；当前实施目标为 S3。后续 S4-S6 在前一切片完成后基于最新代码创建，避免预先固化不再准确的设计。
